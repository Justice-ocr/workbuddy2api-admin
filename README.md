...
- **双版本登录**：国际版固定使用 `www.workbuddy.ai`，国内版使用 `copilot.tencent.com` / `www.codebuddy.cn`；两套 OAuth 会话按 realm 隔离，支持轮询、取消、凭据安全保存及即时加入账号池。
- **账号管理**：查看状态、刷新凭据、停用、恢复、二次确认删除。重复添加已有账号不会覆盖原凭据。
- **运行管理**：查看模型列表、基础运行状态、脱敏运行事件、请求级使用日志与积分任务快照，修改并持久化业务 API Key。使用日志不保存消息正文、请求头或完整上游响应。
...
- **Firefly 前端**：管理台基于 CuteLeaf/Firefly 的 Astro 工程与视觉素材制作，保留本地壁纸、音乐播放器和 Spine 看板娘；音乐默认不自动播放，不加载第三方统计或会话回放脚本。
...
浏览器打开 `http://127.0.0.1:7864/`，输入服务器 `runtime/admin-token` 中的独立管理 Token，再选择国际版或国内版添加账号。令牌不要公开或提交进 Git。
...
`pnpm build` 会把静态产物写入 `cmd/server/admin-dist`，随后由 Go 二进制嵌入。新增安全测试覆盖管理鉴权、静态资源、访问限制和双 realm 登录。生产上线仍需验证实际 OAuth 授权、账号持久化、普通及流式 API；不能仅凭模型列表返回成功判断上游可用。
...
管理台基于 CuteLeaf/Firefly 6.16.8（提交 `567313c1eb357874f3ad03fbae0eb552ede7a06b`）的 Astro 工程和视觉语言制作，使用经作者非商用授权的壁纸、音乐与角色素材。第三方声明见 `web/admin/THIRD_PARTY_NOTICES.md`；这些素材不随本仓库重新授权。页面资源全部本地提供，不启用 Firefly 原站的 Google Analytics、Clarity、51la、Umami、Meting 或其他第三方统计脚本。
...
- 使用日志记录聊天请求的模型、账号 UID、流式模式、状态、耗时、输入/输出 Token 和上游返回的积分。支持 UTC+8 日期范围（单次最多 31 天）、UID、模型、结果筛选和 20/50/100 条分页，并提供成功率、平均耗时、趋势、模型排行、账号排行、请求详情及 CSV 导出。
- CSV 由已鉴权管理接口生成，管理 Token 只通过请求头发送，不进入下载 URL。导出最多 5000 条，并对 `= + - @` 开头的文本做表格公式注入防护。
...
- 上游未返回任务完成时间或业务日期，因此面板采用 UTC+8 本地观测口径：当天首次看到 `completed` 或 `claimed` 后标记“今日已观测完成”，跨日自动重新观测，不声称上游已验证完成日期。
- 任务观测保存在 `state_file` 同目录的 `task-observations.json`，目录权限 `0700`、文件权限 `0600`、原子写入，仅包含 UID、任务代码、状态、进度和观测时间，不包含账号令牌。
